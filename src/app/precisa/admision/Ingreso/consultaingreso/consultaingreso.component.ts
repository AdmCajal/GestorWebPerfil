...
import { ILetrasFacturadas } from '../../../ventas/Contrato/model/iletrasfacturadas';

@Component({
  selector: 'ngx-consultaingreso',
  templateUrl: './consultaingreso.component.html'
})
export class ConsultaingresoComponent extends ComponenteBasePrincipal implements OnInit, UIMantenimientoController {
  ...
  async coreBuscar() {
    ...
    this.contratoService.ReporteIngresoMensual(this.filtro).then((res) => {
      this.bloquearPag = false;
      var contado = 1;
      res.forEach((element) => {
        element.num = contado++;
  ...
  coreVer(dto){
    ...
    async exportExcel() {
      if (this.lstContrato == null || this.lstContrato == undefined || this.lstContrato.length == 0) {
        ...
        let listaExportar: ILetrasVencidas[] = [];
        ...
        this.lstContrato.forEach(function (e) {
          ...
          if (e.FechaPago != null || e.FechaPago != undefined) {
            let fechaPa = new Date(e.FechaPago);
            let dd = ("0" + fechaPa.getDate()).slice(-2);
            let mm = ("0" + (fechaPa.getMonth() + 1)).slice(-2);
            let yyyy = fechaPa.getFullYear();
          ...
          if (e.FechaVencimiento != null || e.FechaVencimiento != undefined) {
            let fechaInicio = new Date(e.FechaVencimiento);
          ...
          let itemExportar: ILetrasFacturadas = {
            Nro: e.num,            
            ...
            LETRA_PEN:new Intl.NumberFormat().format(e.CantidadLetra),
            ESTADO: e.DesEstado, 
            FEC_CANCELADO : fechaFinal,
            FEC_VENC_LETRA: fechaInicial,
            DIAS_MORA: new Intl.NumberFormat().format(e.Dias),
            MORA: new Intl.NumberFormat().format(e.Interes),
            ...
            CELULAR: e.Celular,
            LETRA: e.Nombre,
            NRO_COMPROBANTE: e.CodigoComprobante,
          };
          listaExportar.push(itemExportar);
        });

        const result = await this.exportarService.exportExcel(this.lstContrato, listaExportar, "LetrasPendientes");
        console.log(result);
    ...
    async exportPdf() {
      if (this.lstContrato == null || this.lstContrato == undefined || this.lstContrato.length == 0) {
        ...
        const result = await this.exportarService.ExportPdf(this.lstContrato, col, rows, "LetrasPendientes.pdf", "l");
