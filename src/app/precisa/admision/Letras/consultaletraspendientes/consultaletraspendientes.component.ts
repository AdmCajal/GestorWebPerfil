...
@Component({
  selector: 'ngx-consultaletraspendientes',
  templateUrl: './consultaletraspendientes.component.html'
})
export class ConsultaletraspendientesComponent extends ComponenteBasePrincipal implements OnInit, UIMantenimientoController {
 ...
 /*        this.loteService.listarLote(filtroLote).then(async resp => {
          resp.forEach(async e => {
            if (e.IdLote == element.IdLote) {
    ...
    async exportExcel() {
      if (this.lstContrato == null || this.lstContrato == undefined || this.lstContrato.length == 0) {
        ...
        this.lstContrato.forEach(function (e) {
          ...
          let itemExportar: ILetrasVencidas = {
            Nro: e.num,            
