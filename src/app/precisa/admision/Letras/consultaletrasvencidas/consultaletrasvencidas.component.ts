...
import { IContratoVencido } from '../../../ventas/Contrato/model/icontratovencido';

@Component({
  selector: 'ngx-consultaletrasvencidas',
  templateUrl: './consultaletrasvencidas.component.html'
})
export class ConsultaletrasvencidasComponent extends ComponenteBasePrincipal implements OnInit, UIMantenimientoController {
  ...
  async coreBuscar() {
    ...
    this.contratoService.ReporteContratoAtrasados(this.filtro).then((res) => {
      this.bloquearPag = false;
      var contado = 1;
      res.forEach((element) => {
        element.num = contado++;
  ...
  coreVer(dto){
    ...
    async exportExcel() {
      if (this.lstContrato == null || this.lstContrato == undefined || this.lstContrato.length == 0) {
        ...
        let listaExportar: IContratoVencido[] = [];
        ...
        this.lstContrato.forEach(function (e) {
          ...
          let itemExportar: IContratoVencido = {
            Nro: e.num,
            ...
            LENTRA_VENCIDAS: new Intl.NumberFormat().format(e.CantLetra),
        ...
        const result = await this.exportarService.exportExcel(this.lstContrato, listaExportar, "LetrasVencidas");
        console.log(result);
    ...
    async exportPdf() {
      if (this.lstContrato == null || this.lstContrato == undefined || this.lstContrato.length == 0) {
        ...
        const result = await this.exportarService.ExportPdf(this.lstContrato, col, rows, "LetrasVencidas.pdf", "l");
