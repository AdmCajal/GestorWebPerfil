...
@Component({
  selector: 'ngx-lotescomisionista',
  templateUrl: './lotescomisionista.component.html'
})
export class LotescomisionistaComponent extends ComponenteBasePrincipal implements OnInit, UIMantenimientoController {
  ...
  async coreBuscar() {
    ...
    this.contratoService.ListarLotesxComisionista(this.filtro).then((res) => {
      this.bloquearPag = false;
      var contado = 1;
      res.forEach((element) => {
        element.num = contado++;
  ...
  coreVer(dto){
    ...
    async exportExcel() {
      if (this.lstContrato == null || this.lstContrato == undefined || this.lstContrato.length == 0) {
        ...
        const result = await this.exportarService.exportExcel(this.lstContrato, listaExportar, "Lotes_Comisionista");
        console.log(result);
    ...
    async exportPdf() {
      if (this.lstContrato == null || this.lstContrato == undefined || this.lstContrato.length == 0) {
        ...
        const result = await this.exportarService.ExportPdf(this.lstContrato, col, rows, "Lotes_Comisionista.pdf", "l");
